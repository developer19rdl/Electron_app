...
const ScadaView = () => {
    ...
    const [scadaTabs, setScadaTabs] = useState([
        { name: "Motor 1", image: "", markers: [] },
        { name: "Motor 2", image: "", markers: [] },
        { name: "Motor 3", image: "", markers: [] },
    ]);
    ...
    useEffect(() => {
        const socket = io(process.env.REACT_APP_SOCKET_URL || "http://192.168.31.55:8000", {
    ...
    const handleScadaSuccess = (dataObject) => {
        ...
        setScadaTabs((prevTabs) => {
            ...
            while (updatedTabs.length < maxTabNeeded) {
                updatedTabs.push({ name: `Motor ${updatedTabs.length + 1}`, image: "", markers: [] });
            }
            ...
            scadaData.forEach((item) => {
                ...
                const imageUrl = item.image?.startsWith("http")
                    ? item.image
                    : `${process.env.REACT_APP_RESOURCE_URL || 'http://192.168.31.55:8000/'}${item.image}`;
                ...
                updatedTabs[idx].markers.push({
                    ...
                    x: item.x,
                    y: item.y,
                    label: item.titleName || "Unnamed Sensor",
                    unit: item.units || "",
                    tagKey: item.tagsId,
    ...
    const handleAddMarker = () => {
        ...
        const imagePathToSend = tabImage.startsWith(process.env.REACT_APP_RESOURCE_URL || "http://192.168.31.55:8000/")
            ? tabImage.replace(process.env.REACT_APP_RESOURCE_URL || "http://192.168.31.55:8000/", "")
            : tabImage;
